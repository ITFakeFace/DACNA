...
const AcademicAdminCourseFormPage = () => {
  const { id } = useParams();
      ...
      const fetchCourse = async () => {
        try {
          ...
          if (res.status) {
            ...
            setTimeout(() => navigate('/academic-admin/courses'), 1500);
          }
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setTimeout(() => navigate('/academic-admin/courses', { state: { refresh: true } }), 1500);
      } else {
  ...
  return (
    <div className="p-5 max-w-2xl w-full mx-auto">
      <div className="mb-4 flex items-center gap-4">
        <Button icon="pi pi-arrow-left" severity="secondary" className="!bg-transparent !border-none !text-black" onClick={() => navigate('/academic-admin/courses')} />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
        ...
        <div className="text-right flex gap-3 justify-end">
          ...
          <Button type="button" label="Cancel" className="p-button-secondary" onClick={() => navigate('/academic-admin/courses')} />
        </div>
      </form>
    </div>
  );
...
export default AcademicAdminCourseFormPage;
